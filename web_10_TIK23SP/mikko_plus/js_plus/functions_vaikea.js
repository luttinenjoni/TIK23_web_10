...
function nextQuestion(){
    ...
    let firstNum = Math.floor(Math.random() * 1001); //lausekkeen random luvut välillä 0-100
    let secondNum = Math.floor(Math.random() * 1001); // ^ HOX! +1 pitää lisätä, jotta 0 lasketaan mukaan! myös vastauksiin!
    correctAnswer = firstNum + secondNum; //vastaus
    lausekeDiv.innerHTML = firstNum + "+" + secondNum; //lauseke

    let wrongAnswer1 = Math.floor(Math.random() * 2001); 
    let wrongAnswer2 = Math.floor(Math.random() * 2001); 
    let wrongAnswer3 = Math.floor(Math.random() * 2001); 
    let wrongAnswer4 = Math.floor(Math.random() * 2001); 
